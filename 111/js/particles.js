function launchParticlesJS(tag_id, params) {
    ...
    if (params) {
        if (params.particles) {
            ...
            if (paramsForParticles.line_linked) {
                ...
                if (paramsForLineLinked.enable_auto == false) pJS.particles.line_linked.enable_auto =
                ...
                if (paramsForLineLinked.condensed_mode) {
                    ...
                    if (paramsForCondensedMode.rotateX) pJS.particles.line_linked.condensed_mode.rotateX =
                        paramsForCondensedMode.rotateX;
                    if (paramsForCondensedMode.rotateY) pJS.particles.line_linked.condensed_mode.rotateY =
        ...
        if (params.interactivity) {
            ...
            if (paramsForInteractivity.mouse) {
                if (paramsForInteractivity.mouse.distance) pJS.interactivity.mouse.distance = paramsForInteractivity.mouse
                    .distance;
            }
            ...
            if (paramsForInteractivity.line_linked) {
                if (paramsForInteractivity.line_linked.opacity) pJS.interactivity.line_linked.opacity =
    ...
    pJS.fn.particle = function (color, opacity, position) {
        ...
        this.draw = function () {
            pJS.canvas.ctx.fillStyle = 'rgba(' + this.color.r + ',' + this.color.g + ',' + this.color.b + ',' +
                this.opacity + ')';
                    ...
                    detect_el.onclick = function (e) {
                        if (pJS) {
                            for (var i = 0; i < pJS.interactivity.events.onclick.nb; i++) {
                                pJS.particles.array.push(new pJS.fn.particle(pJS.particles.color_rgb, pJS.particles
                                    .opacity, {
    ...
    pJS.fn.vendors.interactivity.grabParticles = function (p1, p2) {
        ...
        if (dist <= pJS.particles.line_linked.distance && dist_mouse <= pJS.interactivity.mouse.distance && pJS.interactivity
            .status == 'mousemove') {
            var color_line = pJS.particles.line_linked.color_rgb_line;
            pJS.canvas.ctx.beginPath();
            pJS.canvas.ctx.strokeStyle = 'rgba(' + color_line.r + ',' + color_line.g + ',' + color_line.b + ',' + (
